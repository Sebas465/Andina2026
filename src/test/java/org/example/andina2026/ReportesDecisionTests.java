...
class ReportesDecisionTests {
    ...
    String admin, especialista, local, adminEscuela;
    long colA, colB, aulaA, aulaB, rolAlumno, rolDocente, profe, periodo, mate, comu, arte, grado;

    private String login(String dni, String p) throws Exception {
        String b = mvc.perform(post("/login").contentType(MediaType.APPLICATION_JSON)
                .content("{\"dni\":\"" + dni + "\",\"password\":\"" + p + "\"}")).andReturn().getResponse().getContentAsString();
        Matcher m = Pattern.compile("\"token\"\\s*:\\s*\"([^\"]+)\"").matcher(b);
        assertThat(m.find()).as("login de " + dni).isTrue();
        return m.group(1);
    }
    ...
    private long alumno(String nombre, long aula, Double nota, String estadoPsico, String lengua) throws Exception {
        String nac = LocalDate.now().minusYears(13).minusDays(20).toString();
        long p = crear(local, "/api/personas", "{\"nombres\":\"" + nombre + "\",\"apellidos\":\"Test\",\"fechaNacimiento\":\"" + nac
                + "\",\"lenguaMaterna\":\"" + lengua + "\",\"idAula\":" + aula + ",\"idRol\":" + rolAlumno + "}");
        if (nota != null) {
            crear(local, "/api/perfiles-academicos", "{\"notas\":" + nota + ",\"idPersona\":" + p
    ...
    void datos() throws Exception {
        ...
        colA = crear(admin, "/api/colegios", "{\"codigoModular\":\"0100001\",\"nombre\":\"IE Rural\",\"departamento\":\"Puno\",\"provincia\":\"P\",\"distrito\":\"D\",\"comunidad\":\"C\",\"tipo_zona\":\"RURAL\"}");
        colB = crear(admin, "/api/colegios", "{\"codigoModular\":\"0100002\",\"nombre\":\"IE Urbana\",\"departamento\":\"Lima\",\"provincia\":\"P\",\"distrito\":\"D\",\"comunidad\":\"C\",\"tipo_zona\":\"URBANA\"}");
        aulaA = crear(admin, "/api/aula", "{\"nombre\":\"A1\",\"seccion\":\"A\",\"capacidad\":6,\"idColegio\":" + colA + "}");
        ...
        // 6 alumnos quechuahablantes en A1 (capacidad 6 → 100 %), notas bajas; 6 castellanohablantes en B1, notas altas
        ...
        for (int i = 0; i < 12; i++) {
            ...
            ids[i] = alumno("Alumno" + (int) notas[i], i < 6 ? aulaA : aulaB, notas[i], psico, i < 6 ? "QUECHUA" : "CASTELLANO");
        }
        alumno("SinNota", aulaB, null, null, "AMBOS");   // sin perfil: no aparece en rankings

        profe = crear(local, "/api/personas", "{\"nombres\":\"Profe\",\"apellidos\":\"Mamani\",\"idAula\":" + aulaA + ",\"idRol\":" + rolDocente + "}");
        periodo = crear(adminEscuela, "/api/periodos", "{\"nombre\":\"2026-I\",\"fechaInicio\":\"2026-03-01\",\"fechaFin\":\"2026-07-31\"}");
        grado = crear(admin, "/api/grados", "{\"nombre\":\"1ro\",\"nivel\":\"Secundaria\"}");
        mate = crear(adminEscuela, "/api/cursos", "{\"nombre\":\"Matemática\",\"area\":\"Ciencias\"}");
        comu = crear(adminEscuela, "/api/cursos", "{\"nombre\":\"Comunicación\",\"area\":\"Letras\"}");
        arte = crear(adminEscuela, "/api/cursos", "{\"nombre\":\"Arte\",\"area\":\"Artes\"}");
        crear(adminEscuela, "/api/asignaciones-docentes", "{\"horasSemanales\":4.0,\"idAula\":" + aulaA + ",\"idCurso\":" + mate
                + ",\"idPeriodo\":" + periodo + ",\"idPersona\":" + profe + ",\"idColegio\":" + colA + "}");
        crear(adminEscuela, "/api/asignaciones-docentes", "{\"horasSemanales\":3.5,\"idAula\":" + aulaA + ",\"idCurso\":" + comu
                + ",\"idPeriodo\":" + periodo + ",\"idPersona\":" + profe + ",\"idColegio\":" + colA + "}");
        long mat = crear(especialista, "/api/materiales", "{\"titulo\":\"Guía\",\"idPersona\":" + profe + "}");
